...
/***/ function(module, exports, __webpack_require__) {
	...
	function createBox(row, column, x, y) {
	  var letter_id = generateLetterId();
	  var letter = String.fromCharCode(letter_id);
	  var rect = createRect(bg_colors[letter_id - 65]);
	...
	function neighbours(rect) {
	  var lastRow = currentlySelected[currentlySelected.length - 1].row;
	  var lastColumn = currentlySelected[currentlySelected.length - 1].column;
	  var row = rect.row;
	  var column = rect.column;
	  return lastRow - 1 <= row && row <= lastRow + 1 && lastColumn - 1 <= column && column <= lastColumn + 1;
	}
	...
	function init() {
	  ...
	  stage.on("stagemouseup", function (evt) {
	    ...
	    if (currentlySelected.length >= 3) {
	      ...
	      for (var i = 0; i < currentlySelected.length; ++i) {
	        var rect = currentlySelected[i];
	        var removedRow = rect.row;
	        var removedColumn = rect.column;
	        stage.removeChild(rect.parent);
	        currentlySelected[i] = null;

	        for (var j = removedRow - 1; j >= 0; --j) {
	  ...
	  for (var _row = 0; _row < 6; ++_row) {
	    matrix[_row] = [];
	    for (var i = 0; i < 8; ++i) {
	      var rect = createBox(_row, i, start_x + 55 * i, 15 + 60 * _row);
	      matrix[_row][i] = rect;
	    }
	  }
	}
